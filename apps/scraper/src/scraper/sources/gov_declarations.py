"""High-level official property declarations (КПКОНПИ)."""
...
from ..spheres import CATEGORY_DECLARATIONS, SPHERE_GOVERNMENT
from .base import RawPayload, Source


class GovDeclarationsSource(Source):
    id = "gov_declarations"
    raw_ext = "html"
    sphere = SPHERE_GOVERNMENT
    category = CATEGORY_DECLARATIONS

    def _pages(self) -> list[str]:
        raw = os.environ.get("GOV_DECLARATIONS_PAGES", "")
    ...
    def parse(self, payload: RawPayload) -> Iterator[IngestRecord]:
        ...
        for table in soup.find_all("table"):
            ...
            for row in table.find_all("tr"):
                ...
                official = (
                    row_data.get("Име")
                    or row_data.get("Лице")
                    or row_data.get("col_0")
                )
                if not official or official.lower() in {"лице", "име", "три имена"}:
                    continue
                ...
                payload_data = {
                    ...
                    "institution": row_data.get("Институция") or row_data.get("col_2"),
