...
def _ted(payload: dict) -> list[str]:
    # Canonical v2: authority is the top-level block, CPV lives under `tender`.
    authority = (payload.get("authority") or {}).get("name", "")
    tender = payload.get("tender") or {}
    return [payload.get("title", ""), authority, _cpv(tender.get("cpv_code"))]
...
def _caiseop(payload: dict) -> list[str]:
    ...
    tender = payload.get("tender") or {}
    return [payload.get("title", ""), authority, winner, _cpv(tender.get("cpv_code"))]


def _sebra(payload: dict) -> list[str]:
    # Canonical v2: spender/recipient/purpose live under the `payment` block.
    payment = payload.get("payment") or {}
    return [
        payment.get("purpose", "") or payload.get("title", ""),
        payment.get("spender", ""),
        payment.get("recipient", "") or "",
    ]
