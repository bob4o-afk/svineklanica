...
# Concurrency ceiling: the analyzer runs records concurrently AND every Gemini
# call is gated by a global semaphore of this size, so we never have more than
# AGENTS_CAP in-flight LLM agent calls at once (protects quota/rate/cost).
DEFAULT_AGENTS_CAP = 8
# Total budget: the agents make at most this many LLM evaluations per run. Once
# spent, remaining records fall back to deterministic-only (honestly logged).
# Set AGENTS_EVAL_CAP=0 for unlimited.
DEFAULT_AGENTS_EVAL_CAP = 100
...
def _env_int(names: tuple[str, ...], default: int) -> int:
    # Accept the first valid alias (lets us tolerate the AGENDS_CAP typo so a
    # misspelt .env doesn't silently fall back to the default).
    for name in names:
        raw = os.environ.get(name)
...
def load_config() -> AnalyzerConfig:
    ...
    return AnalyzerConfig(
        ...
        agents_cap=_env_int(("AGENTS_CAP", "AGENDS_CAP"), DEFAULT_AGENTS_CAP),
        agents_eval_cap=_env_int(("AGENTS_EVAL_CAP",), DEFAULT_AGENTS_EVAL_CAP),
