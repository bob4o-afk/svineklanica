...
def _analyze_sphere_batch(
    ...
    runnable_cache: dict[str, RunnableParallel] = {}
    ...
    for view in views:
        resolved_sphere = resolve_sphere(view) or sphere
        flow_key = route_flow(view, client, resolved_sphere)
        runnable = _runnable_cache(client, ctx, runnable_cache, flow_key, resolved_sphere)
        ...
        by_source.setdefault(view.source, []).append(verdict)
...
def main() -> None:
    ...
    for source in sources:
        ...
        for view in views:
            ...
            run = _runnable_cache(client, ctx, runnable_cache, fk, sphere)
            verdicts.append(
                analyze_view(
                    view,
                    ctx,
