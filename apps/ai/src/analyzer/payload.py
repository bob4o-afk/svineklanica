...
def view_from_record(record: dict) -> TenderView:
    ...
    value_amount, value_currency = _amount_of(_first(payload, ("value", "amount", "price", "стойност")))
    estimated, _ = _amount_of(_first(payload, ("estimated_value", "estimated", "прогнозна_стойност")))
    final, _ = _amount_of(_first(payload, ("final_value", "final_amount", "paid", "contract_value")))

    bidders = _parse_bidders(payload)
    bids_count = payload.get("bids_count") or payload.get("offers_count")
    if bids_count is None and bidders:
        bids_count = len(bidders)

    subs_raw = payload.get("subcontractors") or []
    ...
    return TenderView(
        ...
        payload=payload,
        title=_name_of(_first(payload, _FIRST_KEYS_TITLE)) or clean_text(payload.get("title")),
        buyer_name=_name_of(_first(payload, _FIRST_KEYS_BUYER)),
        buyer_eik=_eik_of(_first(payload, _FIRST_KEYS_BUYER)),
        winner_name=_name_of(_first(payload, _FIRST_KEYS_WINNER)),
        winner_eik=_eik_of(_first(payload, _FIRST_KEYS_WINNER)),
        value_amount=value_amount,
        value_currency=value_currency,
        estimated_amount=estimated,
        final_amount=final,
        cpv=extract_cpv(str(payload.get("cpv"))) if payload.get("cpv") else None,
        procedure_type=clean_text(str(_first(payload, _FIRST_KEYS_PROCEDURE) or "")),
        status=clean_text(str(_first(payload, _FIRST_KEYS_STATUS) or "")),
        published_at=parse_date(payload.get("published_at") or payload.get("published")),
        deadline=parse_date(payload.get("deadline")),
        award_date=parse_date(payload.get("award_date") or payload.get("awarded_at")),
        signed_at=parse_date(payload.get("contract_signed_at") or payload.get("signed_at")),
        bids_count=int(bids_count) if isinstance(bids_count, (int, float)) else None,
        bidders=bidders,
        subcontractors=subs,
        full_text=_build_full_text(source, payload),
    )
