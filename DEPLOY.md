...
#   docker-compose.prod.yml, Caddyfile, .env
cp .env.prod.example .env     # production template — then fill every <REPLACE_ME>:
```
In `.env` set (all the `<REPLACE_ME>` slots in `.env.prod.example`):
...
- The SSH deploy key is NOT in `.env` — its private half goes in the GitHub secret `DEPLOY_SSH_KEY`.
...
## 5. (Optional) Auto-deploy on every release tag
In the GitHub repo:
...
Now `git tag v1.0.1 && git push --tags` → Actions builds, the VM pulls + migrates automatically, and you get the release email.
...
```
- **Grafana** :3000 (dashboards; login `GRAFANA_ADMIN_*` from .env) ← Prometheus ← cAdvisor (containers) + node-exporter (host).
